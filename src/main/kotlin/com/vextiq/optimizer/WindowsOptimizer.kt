...
class WindowsOptimizer {
    ...
    fun setTimerResolution(onLog: (String) -> Unit) {
        onLog("[>>] Setting timer resolution to 0.5ms...")
        try {
            runCmd("powershell", "-Command", "bcdedit /set useplatformtick yes")
            runCmd("powershell", "-Command", "bcdedit /set disabledynamictick yes")
            onLog("[OK] Timer resolution optimized!")
        } catch (e: Exception) {
            onLog("[!] Timer resolution requires admin rights")
        }
    }
    ...
    fun disableCoreParking(onLog: (String) -> Unit) {
        ...
        try {
            ...
            onLog("[OK] Core parking disabled!")
        } catch (e: Exception) {
            onLog("[!] Core parking requires admin rights")
        }
    }
    ...
    fun setUltimatePowerPlan(onLog: (String) -> Unit) {
        onLog("[>>] Setting ultimate power plan...")
        setHighPerformancePower(onLog)
    }
